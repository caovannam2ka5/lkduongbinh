...
app.post('/api/codelink', (req, res) => {
    try {
        ...
        // Validation
        if (!code || !link) {
            return res.status(400).json({ error: 'Code and link are required' });
        }
        
        if (typeof code !== 'string' || typeof link !== 'string') {
            return res.status(400).json({ error: 'Code and link must be strings' });
        }
        ...
        res.json({ 
            success: true, 
            message: 'Code and link saved successfully',
            data: newEntry 
        });
    } catch (error) {
